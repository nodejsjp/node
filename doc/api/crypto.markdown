...
暗号化された HMAC コンテントを生成するためのクラスです。

`crypto.createHmac()` によって返されます。
...
`crypto.createDecipher` または `crypto.createDecipheriv` から返されます。
...
ディフィー・ヘルマン鍵交換を生成するためのクラスです。

`crypto.createDiffieHellman()` から返されます。
